...
/**
 * Whether a container state is compatible with a transition already in progress
 * — i.e. tells us nothing we did not already know.
 *
 ...
 *
 * - `stopping`: from the SIGTERM until the process exits — the whole point of a
 *   graceful stop, and many seconds of it for a game server saving a world —
 *   `docker inspect` still reports `running`.
