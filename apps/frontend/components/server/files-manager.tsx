...
export function FilesManager({ serverId }: { serverId: string }) {
  ...
  // The file currently open in the in-place editor. `null` shows the listing.
  // Rendered with `key={editing.path}` so switching files resets editor state.
    ...
    return (
      <FileEditor
        ...
        onSave={async (contents) => {
          await handleSaveFile(editing, contents);
          setEditing({ ...editing, sizeBytes: new Blob([contents]).size });
        }}
      />
    );
  }
