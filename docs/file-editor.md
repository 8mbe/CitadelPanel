...
The editor uses the same endpoints the rest of the file manager uses. Nothing
about the backend changed:
...
3. The agent resolves the path through `paths.ts` (the containment
   boundary, where `..` and symlink escapes are caught) and returns the
...
`@monaco-editor/react`, which loads Monaco from a CDN by default. That is not
something a self-hosted panel should depend on, and a needless dependency for a
lifecycle this small. The editor is created once and reconfigured through
`updateOptions` and its model; callbacks are read through a ref so a parent
re-render never rebuilds it; and the controlled `value` is only pushed in when
it diverges from the editor's own text (i.e. a different file was opened, since
typing round-trips through `onChange` and never moves the cursor).
...
them. There is still one palette rather than a second one for the editor.
...
containment boundary, and this just keeps a mangled URL showing a listing
instead of an error banner.

## Code layout

- `components/code-editor.tsx`: the Monaco wrapper. Worker wiring, editor
