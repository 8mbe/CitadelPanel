...
How a server connects to another of the owner's servers, through the "Connect
servers" button in the Settings tab. Covers the pairwise link model, why the
address is a stable name and never an IP, and how links survive container
recreation.

## The flow

A link is an explicit, owner-initiated connection between exactly two servers.
It is what a proxy (Velocity/BungeeCord) needs to reach its backends, or a
plugin to reach another server. The built-in Velocity blueprint is the worked
example: `velocity-proxy.md` covers the other half, turning a link's address
into a `servers` entry the proxy will use. The Settings tab's "Connected
servers" card lists the links and shows a copyable address for each; the
"Connect servers" dialog picks one of the caller's other servers and the
published port to use.

Panel routes (all under `routes/servers.ts`, registered in the dispatcher):

- `GET /api/servers/:id/links` is gated by the `settings` permission (same
  reads-gate-with-writes rule as ports; the addresses reveal host ports).
- `POST /api/servers/:id/links` `{ targetId }` and
  `DELETE /api/servers/:id/links/:linkId` need **owner-or-admin of both
...
- **Same node → internal**: both containers are attached to their pairwise
  ...
  user-defined networks, and the name derives from the server id, so it
  survives every recreate. The port is the peer's published port, which is
...
- **Different node → external**: the two nodes share no Docker daemon, so
  ...
  links, which are a recorded address rather than a network.
...
Each server container lives on its own bridge with ICC disabled, so it "can
reach neither the control-plane nor another tenant's container"
...
- The network is created with **ICC enabled**, which is the whole feature. It
  holds exactly the two linked containers, so a compromised server can only
  ever reach servers its owner explicitly linked it to, never a third tenant.
