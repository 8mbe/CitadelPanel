...
**Self-service (owner)** — `/settings` → *API keys*, backed by the plugin's
own endpoints (`/api/auth/api-key/list|create|delete`). A key lists, mints,
and revokes only the caller's own keys.

**Admin oversight** — `/admin/api-keys`, backed by panel routes (the plugin
has no cross-user surface, so these query the `apikey` table directly like the
other admin list routes):
...
| `PATCH /api/admin/api-keys/:id` | enable/disable any key — reversible, preserves the row and counters |
