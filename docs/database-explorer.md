...
column *names* and key values, never full row data.
...
string `"NULL"` identically — a row edit keyed on a mis-read value would
...
- `components/server/db-explorer/db-explorer.tsx` — the shell: table sidebar
  (name, ≈ row estimate, size), table picker, create-table dialog trigger.
