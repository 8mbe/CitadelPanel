...
- A background watcher heuristically scores containers for cryptomining/abuse behavior and maintains a **suspicious activity list** for admin review (auto-kill is optional/off by default — detection isn't perfect, so we flag rather than blindly act).
...
| Auth | **Better Auth** (email/password + optional OAuth + 2FA plugin) | Battle-tested session/credential handling instead of hand-rolled JWT/Argon2id — less surface area for auth bugs |
...
All secrets (`server_env` values holding tokens/API keys) are encrypted with a panel-wide key (AES-256-GCM) before storage — never stored in plaintext even in the DB.

## 5. Roles & Permissions Model

Exactly **2 global roles**, kept simple on purpose:

- **`admin`** — full control: manage all servers (including their resource limits), manage nodes, review suspicious activity, manage users, manage game presets.
...
- Session cookies are httpOnly, `SameSite=Strict`, Secure in production — Better Auth's default cookie-session handling, no custom JWT signing needed.
...
- `nodes` table stores each node's connection info (`docker_endpoint`, TLS client cert/key/CA — encrypted at rest) plus reported capacity (`cpu_total`, `memory_total_mb`, `disk_total_mb`) and a `last_heartbeat_at` for health monitoring.
...
- Explicit CPU (`--cpus`) and memory (`--memory`) hard limits — no bursting beyond the server's plan, which also bounds the blast radius of any undetected miner.
...
- Optional (config flag, off by default): **auto-suspend** if score exceeds a hard emergency threshold — opt-in per deployment, since false positives are costly for legitimate heavy workloads. The threshold must stay below the 130-point ceiling the weights allow, or enabling it would have no effect.
...
- The shared per-node database server (§7.1) relies on disabled inter-container communication + custom `DOCKER-USER` iptables rules rather than pure Docker network isolation — more fragile than a fully isolated per-server DB, and must be covered by an integration test asserting cross-tenant unreachability on `node_db_net`.
...
- **An admin cannot change their own role, and the last admin cannot be demoted** — no accidental lockout.
...
2. **`/servers/[id]`** — the server page: name/status/power actions header, then sections as real routes: console (stats cards for CPU/memory/disk/players + interactive console), files, database, ports, subusers, settings, activity. Files/database/ports are placeholders; the rest carry over the previously built UI. `/servers/[id]` itself redirects to `/servers/[id]/console` so both addresses reach the console.
...
- Server tiles on `/` got quick actions in the footer: **Files**, **Players**, and **Start** (shown only when the server is stopped/suspended/error — same gating as the server page). The card title is now a stretched link so the footer buttons remain clickable.
